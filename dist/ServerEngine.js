...
class ServerEngine {
    ...
    step() {
        ...
        if (this.requestImmediateUpdate || this.gameEngine.world.stepCount % this.options.updateRate === 0) {
            ...
            for (let socketId of Object.keys(this.connectedPlayers)) this.connectedPlayers[socketId].socket.emit('worldUpdate', payload);
        ...
        if (this.gameEngine.trace.length) {
            ...
            _fs2.default.appendFile(`${this.options.tracesPath}server.trace`, traceString, err => {
                if (err) throw err;
            });
        }
    }
    ...
    onPlayerConnected(socket) {
        ...
        socket.on('trace', function (traceData) {
            ...
            _fs2.default.appendFile(`${that.options.tracesPath}client.${playerId}.trace`, traceString, err => {
                if (err) throw err;
            });
        });
