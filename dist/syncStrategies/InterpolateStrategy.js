...
var _SyncStrategy = require('./SyncStrategy');

var _SyncStrategy2 = _interopRequireDefault(_SyncStrategy);
...
class InterpolateStrategy extends _SyncStrategy2.default {
    ...
    interpolate() {
        ...
        if (stepEvents && stepEvents.objectUpdate) {
            stepEvents.objectUpdate.forEach(ev => {
                if (!world.objects[ev.objectInstance.id]) this.addNewObject(ev.objectInstance.id, ev.objectInstance, stepToPlay);
            });
        }
        ...
        if (stepEvents && stepEvents.objectDestroy) {
            stepEvents.objectDestroy.forEach(ev => {
                if (world.objects[ev.objectInstance.id]) this.gameEngine.removeObjectFromWorld(ev.objectInstance.id);
            });
        }
        ...
        world.forEachObject((id, ob) => {
            ...
            if (nextObj) {
                ...
                if (this.options.reflect) playPercentage = 1.0;
