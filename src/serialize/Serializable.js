...
export default class Serializable {
    ...
    serialize(serializer, options) {
        ...
        if (netScheme) {
            for (let property of Object.keys(netScheme).sort()) {
                ...
                if (netScheme[property].type === TYPES.STRING) {
                ...
                } else if (netScheme[property].type == TYPES.CLASSINSTANCE) {
                ...
                } else if (netScheme[property].type == TYPES.LIST) {
                    ...
                    for (let item of this[property]) {
                        ...
                        if (netScheme[property].itemType == TYPES.CLASSINSTANCE) {
    ...
    prunedStringsClone(serializer, prevObject) {
        ...
        let isString = p => netScheme[p].type === TYPES.STRING;
    ...
    syncTo(other) {
        ...
        for (let p of Object.keys(netScheme)) {
            ...
            if (netScheme[p].type === TYPES.LIST || netScheme[p].type === TYPES.CLASSINSTANCE)
                continue;

            // strings might be pruned
            if (netScheme[p].type === TYPES.STRING) {
